...
def create_dashed_lines():
    ...
    curve_objs = [obj for obj in selected if obj.type == 'CURVE']
    if not curve_objs:
        print("❌ 请至少选中一条曲线")
        return

    print(f"✅ 找到 {len(curve_objs)} 条曲线，开始批量生成...")

    # ---------- 清理旧资源（避免累积） ----------
    old_seg = bpy.data.objects.get("Dashed_Segment")
    if old_seg:
        bpy.data.objects.remove(old_seg, do_unlink=True)
    old_group = bpy.data.node_groups.get("Dash_Placement")
    if old_group:
    ...
    half_thk = 0.025   # 厚度固定0.05米

    bm = bmesh.new()
    verts = [
        Vector((-half_wid, -half_thk, -half_len)),
        Vector(( half_wid, -half_thk, -half_len)),
        Vector(( half_wid,  half_thk, -half_len)),
        Vector((-half_wid,  half_thk, -half_len)),
        Vector((-half_wid, -half_thk,  half_len)),
    ...
    print(f"   📁 共处理 {len(curve_objs)} 条曲线")
